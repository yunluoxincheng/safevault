...
**Android 前端（本地加密/解密 - 自适应配置）**
- 标准配置：128MB 内存，3 次迭代，4 并行
- 最低下限：64MB 内存，2 次迭代，2 并行
- 自适应逻辑：根据设备内存（25%）和 CPU 核心数自动调整
- 输出长度：32 字节 (256 位)
- 盐值长度：16 字节 (128 位)

**Android 前端（云端同步 - 固定高配）**
- 内存：131072KB (128MB)
- 迭代次数：3
- 并行度：4
- 确保跨设备密钥派生结果一致
...
- **特点**: 主密码相同，派生出的密钥必相同
- **参数**: 自适应配置（标准 128MB/3次/4并行，最低 64MB/2次/2并行）
...
| 参数 | 值 | 说明 |
|------|-----|------|
| **Argon2id timeCost** | 3 (标准) / 2 (最低) | 迭代次数 |
| **Argon2id memoryCost** | 自适应 (标准128MB, 最低64MB) | 本地内存消耗 |
| **Argon2id memoryCost (云端)** | 131072KB (128MB) | 云端同步固定值 |
| **Argon2id parallelism** | 4 (标准) / 2 (最低) | 并行线程数 |

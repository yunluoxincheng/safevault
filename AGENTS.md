...
SafeVault 是课程期末作业项目：一个 Android 密码管理器，采用 monorepo 布局，包含原生 Android 客户端和 Spring Boot 后端。

- Android 客户端在 `android/`（模块在 `android/app/`），主语言 Java 17，界面以 XML + ViewBinding 为主，使用 Material Components、ConstraintLayout、Navigation Component、LiveData/ViewModel、Retrofit/OkHttp、RxJava、Room、AndroidX Biometric、ZXing、Bouncy Castle、Argon2Kt。
- 后端在 `server/`，Spring Boot 3.5.9 + Java 17 + Maven，使用 Spring Security、JWT、JPA、Flyway、PostgreSQL、Redis、WebSocket/STOMP、SpringDoc OpenAPI、Bucket4j、Bouncy Castle、Argon2-JVM。后端由 root Git 直接跟踪，不再是嵌套独立 Git 仓库。
- 当前 Android 包名和 namespace 是 `com.ttt.safevault`。Manifest 使用 `.core.SafeVaultApplication`，源文件路径在 `android/app/src/main/java/com/ttt/safevault/SafeVaultApplication.java`。
- 当前后端根包是 `org.ttt.safevaultbackend`。
- `docs/` 存放主要项目文档，`server/docs/` 存放后端部署/模块化文档，`openspec/` 存放规格和变更提案。
- `android/.gradle/`、`android/build/`、`android/app/build/`、`server/target/` 是生成物目录，探索时通常忽略。
...
- Android 单元测试：在 `android/` 下运行 `./gradlew test`
- Android 编译：在 `android/` 下运行 `./gradlew :app:assembleDebug`
- 后端测试：在 `server/` 下运行 `./mvnw test`
- 后端启动：在 `server/` 下运行 `./mvnw spring-boot:run`
- 后端依赖服务：在 `server/` 下运行 `docker compose up -d postgres redis`
...
## Documentation Layout Update (2026-05-13)

- Repository is now a monorepo: `android/` and `server/` tracked by root Git.
...
- Backend routing policy:
  ...
  - `server/docs/` for backend-local deployment/operations.
